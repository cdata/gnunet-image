...
RUN git clone https://git.gnunet.org/gnunet.git && cd gnunet && git checkout $GNUNET_REVISION && ./bootstrap && ./configure --prefix=/opt --disable-documentation --with-microhttpd --with-extractor=/opt --with-libgnurl=/opt && make && make install && rm -rf /opt/gnunet/

FROM alpine:edge

WORKDIR /opt

RUN apk add --update bash ca-certificates openssl libbz2 libtool libltdl libunistring libidn jansson libmicrohttpd gnutls sqlite-dev glib libgcrypt gmp argon2 libsodium && update-ca-certificates && rm -rf /var/cache/apk/* /tmp/*
